...
private fun createCompositeTrustManager(): X509TrustManager {
    ...
    val bundledKs = KeyStore.getInstance(KeyStore.getDefaultType()).apply { load(null, null) }
    val cf = CertificateFactory.getInstance("X.509")
    for ((alias, pem) in bundledCerts()) {
        bundledKs.setCertificateEntry(alias, cf.generateCertificate(ByteArrayInputStream(pem.toByteArray())))
    }
    val bundledTmf = TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm()).apply {
        init(bundledKs)
    }
...
private fun bundledCerts(): List<Pair<String, String>> = listOf(
    "isrg_root_x1" to ISRG_ROOT_X1,
    "isrg_root_x2" to ISRG_ROOT_X2
)
