...
fun AppUpdateGate(content: @Composable () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        state = try {
            val config = HttpClientFactory.publicClient
                .get("/api/v1/app-config/minimum-version?app=carryon&platform=${appPlatform()}") {
...
private fun MandatoryUpdateScreen(storeUrl: String?) {
    ...
    Surface(Modifier.fillMaxSize()) {
        Column(
            ...
            Text(
                "A newer version of CarryOn is required to continue.",
